...
const schema = {
  description: 'Updates a club diving cylinder set',
  tags: ['Club cylinder'],
...
const handler = async (
  ...
  await knexController.transaction(async (trx) => {
    // Check that the cylinder set exists and is a club cylinder
    const exists = await clubCylinderSetExists(setId, trx);
    if (!exists) {
      await trx.rollback();
    ...
    if (!updateResult) {
      log.error(
        `Tried to update nonexisting club cylinder set even though it should exist. Id: ${setId}`,
      );
