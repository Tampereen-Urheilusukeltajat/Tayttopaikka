...
Notify a user by email when an admin changes their `isUser` access right via `PATCH /users/:userId/roles`, so they know when their account is activated or deactivated.
